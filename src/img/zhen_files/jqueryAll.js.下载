 ...
 * 澧炲姞FireFox 涓嬩腑鏂� 鐨勬敮鎸� 鍦�198 琛屽姞浜嗕竴涓狟IND 璇彞
		...
		switch(event.keyCode) {
			...
			case KEY.RETURN:
				if( selectCurrent() ) {
					...
					event.preventDefault();
					blockSubmit = true;
					//searchForm();//鍥炶溅浜嬩欢鑷姩鎼滅储 2014.1.13鏃ヤ慨鏀�
					return false;
				}
				break;
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(lastWord(term) != "") // add by lilongfei
	...
	function init() {
		...
		list = $("<ul/>").appendTo(element).mouseover( function(event) {
			if(target(event).nodeName && target(event).nodeName.toUpperCase() == 'LI') {
	            ...
	            // add by lilongfei start
	            $("li", list).find(".his a").html("搜索历史");
	            $(target(event)).find(".his a").html("删除");
	            // add by lilongfei end
